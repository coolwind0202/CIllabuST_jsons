...
if __name__ == "__main__":
    ...
    subject_category_classifier = category.SubjectCategoryClassifier(
        pe_possibility_detector=category.PEPossibilityDetectorByCellTexts(
            pe_page_cell_texts
        ),
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        file_subjects = [
            [parser.parse_chunk(filename=unquote(url), chunk=chunk) for chunk in chunks]
            for url, chunks in zip(nav.syllabus_urls, file_chunks)
        ]
        syllabus = models.Syllabus(subjects=more_itertools.flatten(file_subjects))

        logger.info("Dump syllabus to dist files")
        dump = syllabus.model_dump_json()
        schema = json.dumps(syllabus.model_json_schema())
